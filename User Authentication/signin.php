...
if(count($_POST)> 0){
    ...
    if(strlen($error)==0){
        ...
        while(!feof($fp)){
            $line=fgets($fp);
            $line=explode(';',$line);
            if(count($line)==3 && $_POST['email']==$line[1] && password_verify($_POST['password'],trim($line[2]))){
                $_SESSION['username']=$line[0];
                header('location: ../Entity Horror/index.php');
                die();
            } 
        }
        fclose($fp);
            ...
            <!-- Sign Up Form -->
                <form class="col-lg-6 offset-lg-3" method="POST"><?php if(strlen($error)>0) echo '<span style="color:#f30">'.$error.'</span>'; ?>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mb-3"><button type="submit" class="btn btn-primary">Sign In</button>
                    </div>
